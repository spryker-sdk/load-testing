...
trait AddOfferToCustomerCartBase {
    ...
    val loginPageRequest = http("Login Page Request")
        ...
        .check(status.is(200))
        .check(
            css("input[name=\"loginForm[_token]\"]", "value").saveAs("loginFormCsrfToken")
        )
...
class AddOfferToCustomerCartRamp extends Simulation with AddOfferToCustomerCartBase {

    override lazy val scenarioName = "Add Offer To Customer Cart [Incremental]"
...
class AddOfferToCustomerCartSteady extends Simulation with AddOfferToCustomerCartBase {

    override lazy val scenarioName = "Add Offer To Customer Cart [Steady RPS]"
