...
trait CartMerchantTimeslotFullFlowApiBase {

  lazy val scenarioName = "Cart Merchant Timeslot Full Flow Api"
  ...
  val cartMerchantTimeslotsrequest = http(scenarioName)
    .get("/carts/${cart_id}/cart-merchant-timeslots")
    .header("Authorization", "Bearer ${access_token}")
    .check(status.is(200))
  ...
  val scn = scenario(scenarioName)
    ...
    .pause(3)
    .exec(cartMerchantTimeslotsrequest)
}

class CartMerchantTimeslotFullFlowApiRamp extends Simulation with CartMerchantTimeslotFullFlowApiBase {
...
class CartMerchantTimeslotFullFlowApiSteady extends Simulation with CartMerchantTimeslotFullFlowApiBase {

  override lazy val scenarioName = "Cart Merchant Timeslot Full Flow Api [Steady RPS]"
