...
trait GuestWithOffersCartApiBase {

    lazy val scenarioName = "Add offer to Guest Cart Api"
    ...
    val addToCartRequest = http("Add offer to Guest Cart")
        .post("/guest-cart-items")
        .body(StringBody("""{"data": {"type": "guest-cart-items", "attributes": {"sku": "${product_sku}", "productOfferReference": "${product_offer_reference}", "quantity": 1}}}""")).asJson
...
class GuestWithOffersCartApiRamp extends Simulation with GuestWithOffersCartApiBase {
...
class GuestWithOffersCartApiSteady extends Simulation with GuestWithOffersCartApiBase {
