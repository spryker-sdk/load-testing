...
trait PlaceOrderWithOffersCustomerBase {
    lazy val scenarioName = "Place Order With Offers as Customer"
    ...
    val addToCartRequest = http("Add to Cart request")
        .post("/en/cart/add/${product_sku}")
        .formParam("quantity", "10")
        .formParam("add_to_cart_form[_token]", "${addToCartFormCsrfToken}")
        .formParam("product_offer_reference", "${product_offer_reference}")
...
class PlaceOrderWithOffersCustomerRamp extends Simulation with PlaceOrderWithOffersCustomerBase {

    override lazy val scenarioName = "Place Order With Offers as Customer [Incremental]"
...
class PlaceOrderWithOffersCustomerSteady extends Simulation with PlaceOrderWithOffersCustomerBase {

    override lazy val scenarioName = "Place Order With Offers as Customer [Steady RPS]"
