...
trait UpdateCartFullFlowApiBase {

  lazy val scenarioName = "Update Cart Full Flow Api"
  ...
  val createCartRequest = http("Create Cart Request")
    ...
    .header("Content-Type", "application/json")
    .body(StringBody("""{"data":{"type":"carts","attributes":{"priceMode":"NET_MODE","currency":"USD","store":"US","merchantReference":"${merchant_reference}","merchantSelections":{"serviceType":"delivery","merchantFilterAddress":{"address1":"Kharkiv Ukraine","zipCode":"12345"}},"merchantTimeslot":{"merchantTimeslotId":1,"startTime":"2022-09-16","endTime":"2022-12-18","merchantTimeslotReservation":{"merchantTimeslotReservationId":1,"expirationDate":"2022:03:01-14:37:45"}}}}}""")).asJson
    .check(status.is(201))
  ...
  val updateCartRequest = http("Update Cart Request")
    .patch("/carts/${cart_id}")
    .body(StringBody("""{"data":{"type":"carts","attributes":{"priceMode":"NET_MODE","currency":"USD","store":"US","merchantReference":"474-001","merchantSelections":{"serviceType":"pickup","merchantFilterAddress":{"address1":"Kharkiv Ukraine","zipCode":"12345"}},"merchantTimeslot":{"merchantTimeslotId":1,"merchantTimeslotReservation":{"merchantTimeslotReservationId":1,"expirationDate":"2022:03:01-14:37:45"}},"tipAmount":20}}}""")).asJson
    .header("Authorization", "Bearer ${access_token}")
    .header("Content-Type", "application/json")
...
class UpdateCartFullFlowApiRamp extends Simulation with UpdateCartFullFlowApiBase {

  override lazy val scenarioName = "Update Cart Full Flow Api [Incremental]"
...
class UpdateCartFullFlowApiSteady extends Simulation with UpdateCartFullFlowApiBase {

  override lazy val scenarioName = "Update Cart Full Flow Api [Steady RPS]"
